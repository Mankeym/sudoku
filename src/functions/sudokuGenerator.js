...
export function isValidSudoku(matrix) {
    ...
    for (let i = 0; i < 9; i += 3) {
        for (let j = 0; j < 9; j += 3) {
            ...
            for (let k = i; k < i + 3; k++) {
                for (let l = j; l < j + 3; l++) {
                    block.push(matrix[k][l]);

                }
            }
            if (!checkUniqueDigits(block)) {
              return false;
            }
        }
    }
...
function checkUniqueDigits(sequence) {
    // Создаем объект для хранения встречающихся чисел
    const seen = {};
    for (let digit of sequence) {
        if (seen[digit] || digit < 1 || digit > 9) {
            return false;
            }
        seen[digit] = true;
    }
...
function removeCells(grid) {
    const DIFFICULTY = 30;
    let resultGrid = [...grid].map(row => [...row]);
